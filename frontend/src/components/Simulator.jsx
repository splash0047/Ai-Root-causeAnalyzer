...
export default function Simulator() {
  ...
  const [fixType, setFixType] = useState('impute');

  async function runSim() {
    setLoading(true); setError(null); setResult(null); setFixResult(null);
    try {
  ...
  async function runFixSimulation() {
    setFixLoading(true);
    try {
      const data = await api.simulateFix(result.dataset_records, result.actuals, fixType, feature);
      setFixResult(data);
    } catch (e) { setError(e.message); }
    setFixLoading(false);
  }
    ...
    <div className="animate-in">
      ...
      {benchmarkResult && (
        <div className="cyber-card accent-purple" style={{ padding: 28, marginBottom: 24 }}>
          <h3 style={{ marginBottom: 16, fontWeight: 700, fontFamily: 'var(--font-title)' }}>Synthetic Demo Results</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            {benchmarkResult.map((res, i) => (
              <div key={i} style={{ background: 'rgba(255,255,255,0.05)', padding: 16, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)' }}>
                ...
                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Detected: {res.root_cause}</div>
                  ...
                  <div style={{marginTop: 24, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.1)'}}>
                    ...
                    <div style={{display: 'flex', gap: 12, alignItems: 'center'}}>
                      <select className="form-select" value={fixType} onChange={e => setFixType(e.target.value)} style={{flex: 1}}>
                        <option value="impute">Replace with Training Median</option>
                        <option value="drop">Set Feature to Zero</option>
                      </select>
                      <button className="btn btn-secondary" onClick={runFixSimulation} disabled={fixLoading}>
                    ...
                    {fixResult && (
                      <div style={{marginTop: 16, padding: 16, background: 'rgba(16, 185, 129, 0.1)', borderRadius: 8, border: '1px solid rgba(16, 185, 129, 0.2)'}}>
                        <div style={{color: 'var(--accent-emerald)', fontWeight: 600, marginBottom: 8}}>{fixResult.message} Prediction flip rate: {(fixResult.prediction_flip_rate * 100).toFixed(1)}%.</div>
                        <div style={{display: 'flex', alignItems: 'center', gap: 12}}>
                          <div style={{flex: 1}}>
                            <div style={{fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4}}>Before substitution (synthetic label accuracy): {fixResult.before_accuracy == null ? 'Unavailable' : `${(fixResult.before_accuracy * 100).toFixed(1)}%`}</div>
                            <div style={{height: 8, background: 'rgba(255,255,255,0.1)', borderRadius: 4, overflow: 'hidden'}}>
                              <div style={{height: '100%', width: `${(fixResult.before_accuracy ?? 0) * 100}%`, background: 'var(--accent-rose)'}}></div>
                            </div>
                          </div>
                          <div style={{fontSize: '1.2rem'}}>→</div>
                          <div style={{flex: 1}}>
                            <div style={{fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: 4}}>After substitution (synthetic label accuracy): {fixResult.after_accuracy == null ? 'Unavailable' : `${(fixResult.after_accuracy * 100).toFixed(1)}%`}</div>
                            <div style={{height: 8, background: 'rgba(255,255,255,0.1)', borderRadius: 4, overflow: 'hidden'}}>
                              <div style={{height: '100%', width: `${(fixResult.after_accuracy ?? 0) * 100}%`, background: 'var(--accent-emerald)'}}></div>
                            </div>
                          </div>
                        </div>
                      </div>
